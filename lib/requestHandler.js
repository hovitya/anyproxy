...
function getWSPort(info) {
  if (info.port) return info.port;
  if (info.protocol === 'wss') return 443;
  return 80;
}
...
function getWsHandler(userRule, recorder, wsClient, wsReq) {
  ...
  try {
    ...
    const wsUrl = `${serverInfo.protocol}://${serverInfo.hostName}:${getWSPort(serverInfo)}${serverInfo.path}`;
    const { headers, agent } = userRule.getWSProxyAgent(serverInfo.noWsHeaders)
